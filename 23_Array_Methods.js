...
// find(), Delete All Elements from an array.

// reverse() is discussed in 24_Array_Sort.js
...
// To delete all elements from an array in JavaScript, you can use one of these 
// methods:
// 1. Set the Array's length to 0
// This is the fastest and most efficient way to empty an array:
...
// Works even if the array is declared with const.
...
// Pros:
// Directly modifies the original array.
// Returns the removed elements if needed.

// 3. Use pop() in a Loop
// Iteratively remove the last element (not recommended for large arrays):
...
// Avoid this method for performance-critical code.

// Key Takeaways:
// Best Method: Use arr.length = 0 (efficient and simple).
// Avoid Reassignment: If other parts of your code reference the original array.
// Performance: splice(0) and length = 0 are faster than looping with pop().

// Example with const Arrays
// If the array is declared with const, use methods that modify the array in place:

// const arr = [1, 2, 3, 4];
// arr.length = 0; // Works (modifies in place)
// console.log(arr); // Output: []